...
@section('content')
 ...
 {!! Form::open(['url'=>'object/edit','method'=>'POST'])  !!}
    ...
    <button type="subbmit" class="btn btn-primary btn-sm">Редактировать</button>
 {!! Form::close() !!}
 {!! Form::open(['url'=>'object/delete','method'=>'POST'])  !!}
    <input type="hidden" name="fclient_id" value="{{$user->id}}" >
    <input type="hidden" name="id" value="{{$object->id}}" >
    <button type="subbmit" class="btn btn-primary btn-sm">Удалить объект</button>
 {!! Form::close() !!}
 {!! Form::open(['url'=>'showfb','method'=>'POST'])  !!}
    <input type="hidden" name="fclient_id" value="{{$user->id}}" >
    <input type="hidden" name="id" value="{{$object->id}}" >
    <button type="subbmit" class="btn btn-primary btn-sm">Смотреть отзывы</button>
 {!! Form::close() !!}
 {!! Form::open(['url'=>'changequestions','method'=>'POST'])  !!}
    <input type="hidden" name="id" value="{{$object->id}}" >
    <button type="subbmit" class="btn btn-primary btn-sm">Изменить анкету</button>
 {!! Form::close() !!}
